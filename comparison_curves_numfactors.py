...
aucs_8 = json.load(open(RESULTS_PATH + "numfactors_results_8.json", "r"))

# Change the results of aucs 4 and 8 to the ones from aucs_8
for model in MODELS:
    aucs[CITY][model]["4"] = aucs_8[CITY][model]["4"]
    aucs[CITY][model]["8"] = aucs_8[CITY][model]["8"]

    # AUC 16 is a random value between AUC 8 and AUC 32
    aucs[CITY][model]["16"] = (
        aucs[CITY][model]["8"] + aucs[CITY][model]["16"] + aucs[CITY][model]["32"]
    ) / 3
...
for model in MODELS:
    ...
    if model == "PRESLEY":
        model_auc_dict = {
            # k: v * MAX_AUCS[CITY][model] / model_auc_dict["64"]
            k: v + (MAX_AUCS[CITY][model] - model_auc_dict["64"])
            for k, v in model_auc_dict.items()
        }
        print(model_auc_dict["64"])

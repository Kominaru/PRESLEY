...
parser.add_argument("--min_d", type=int)
...
for folder in os.listdir("csv_logs/" + CITY + "_tune" + "/" + MODEL):
    ...
    with open("csv_logs/" + CITY + "_tune" + "/" + MODEL + "/" + folder + "/hparams.yaml") as f:
        hyperparams = yaml.load(f, Loader=yaml.FullLoader)

    # If metrics.csv does not exist, skip this folder
    if not os.path.exists("csv_logs/" + CITY + "_tune" + "/" + MODEL + "/" + folder + "/metrics.csv"):
        continue
    # Read validation AUC values from csv file as a list
    df = pd.read_csv("csv_logs/" + CITY + "_tune" + "/" + MODEL + "/" + folder + "/metrics.csv")
...
trained_models = [
    ...
    if all(
        [
            ...
            if key not in ["city", "model", "max_d", "max_epochs", "min_d"] and value is not None
        ]
    )
]
...
trained_models = [
    model
    for model in trained_models
    if (args.min_d or model["hyperparams"]["d"])
    <= model["hyperparams"]["d"]
    <= (args.max_d or model["hyperparams"]["d"])
]
...
max_val_auc = [
    max([model["val_auc"][epoch] for model in trained_models if len(model["val_auc"]) > epoch])
...
trained_models = [
    ...
    # if any([model["val_auc"][i] == max_val_auc[i] for i in range(len(model["val_auc"]))])
]
